...
#gerar os graficos
print("Gerando e salvando os gráficos SHAP...")
...
plt.close()
print("  - Gráfico 'shap_feature_importance.png' salvo.")

# grafico de resumo (Beeswarm Plots)
...
if isinstance(shap_values, np.ndarray) and shap_values.ndim == 3:
    ...
    for i, class_name in enumerate(class_names):
        plt.figure()
        #passa os valores shap para a classe 'i'
        ...
        plt.tight_layout()
        plt.savefig(f"shap_summary_plot_class_{class_name}.png", dpi=300, bbox_inches='tight')
        plt.close()
        print(f"  - Gráfico 'shap_summary_plot_class_{class_name}.png' salvo.")
else:
    ...
    plt.tight_layout()
    plt.savefig("shap_summary_plot_geral.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("  - Gráfico 'shap_summary_plot_geral.png' salvo.")
...
for feature_name in top_3_features:
    ...
    shap.dependence_plot(
        feature_name,
        shap_values if shap_values.ndim < 3 else shap_values[:,:,-1], # usamos os shap values da classe de maior grau para interacoes
        X,
        display_features=X,
        interaction_index="auto", #shap escolhe a melhor feature pra colorir
        show=False
    )
    plt.tight_layout()
    plt.savefig(f"shap_dependence_plot_{feature_name}.png", dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  - Gráfico 'shap_dependence_plot_{feature_name}.png' salvo.")


# gerar Decision Plots
# o método .predict() do booster retorna as probabilidades pra cada classe
predicted_probas = model_to_explain_obj.predict(X)

# pegamos a probabilidade da ultima classe (a de maior grau, ex: Classe 5)
proba_high_grade = predicted_probas[:, -1]

try:
    # critérios para aumentar a chance de encontrar exemplos
    ...
    example_indices = {
        "low_grade_example": low_proba_idx,
        "mid_grade_example": mid_proba_idx,
        "high_grade_example": high_proba_idx
    }
    print(f"Índices dos exemplos para os Decision Plots: {example_indices}")

    # gera um Decision Plot para cada exemplo
    # o explainer.expected_value pode ser uma lista para multiclasse
    base_value_for_plot = explainer.expected_value[-1] if isinstance(explainer.expected_value, list) else explainer.expected_value

    # o shap_values também pode ser uma lista de matrizes ou uma matriz 3D
    ...
    for name, idx in example_indices.items():
        shap.decision_plot(
            base_value_for_plot,
            shap_values_for_plot[idx], # os valores shap para este exemplo e esta classe
            X.iloc[idx],
            feature_names=X.columns.tolist(),
            show=False
        )
        plt.tight_layout()
        plt.savefig(f"shap_decision_plot_{name}.png", dpi=300, bbox_inches='tight')
        plt.close()
        print(f"  - Gráfico 'shap_decision_plot_{name}.png' salvo.")
